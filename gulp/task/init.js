...
/**
 * 国际化 : 将i18n/*.json -> i18n.js, 通过 var i18n = {} 写在 index.html中, 等待使用
 * 参考 : https://www.npmjs.com/package/gulp-merge-json
 */
gulp.task('i18n', ['clean-i18n'], function () {
  return gulp.src(config.i18n.src)
    .pipe($.mergeJson({
      fileName: config.i18n.file,
      exportModule: config.i18n.exportModule
    }))
...
/**
 * angular 模板缓存
 * 参考 : https://www.npmjs.com/package/gulp-angular-templatecache
 */
...
gulp.task('libs', ['clean-libs'], function () {
  utils.log('Copying the libs');

  return gulp
    .src(config.libs)
    .pipe(gulp.dest(config.buildContent + 'libs'));
});

/**
 * wiredep : install bower's lib
 */
gulp.task('wiredep', ['i18n'], function () {
  utils.log('Wire up bower css js and app js into the html .');
  var options = config.getWiredepDefaultOptions();
  var i18n = config.tmp + config.i18n.file;
 ...
 */
gulp.task('build', ['optimize', 'images', 'fonts', 'libs'], function () {
